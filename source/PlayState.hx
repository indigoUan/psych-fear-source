...
{
	public var gotSlashed:Bool = false;
	public var letsYell:Bool = false;
	public var yellValue:Float = 0;
	public var shakeIntensity:Int = 4;
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'deranged' | 'adrenaline' | 'phobophobia':
					curStage = 'cont';
		...
		{
			case 'cont': //Week fear
			var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('fear/containment'));
			bg.screenCenter();
			bg.updateHitbox();
			add(bg);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Scream':
				yellValue = Std.parseFloat(value1);
				letsYell = true;

			case 'Set Fear Shake':
				shakeIntensity = Std.parseInt(value1);
		}
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							if(!note.isSustainNote) {
								health -= 0.06; //0.26 + 0.04 = -0.3 (-15%) of HP if you hit a hurt note
	...
	function StrumPlayAnim(isDad:Bool, id:Int, time:Float) {
		...
		if(isDad) {
			camGame.shake((0.001 * shakeIntensity), 0.1);
	...
	public function poison():Void {
		canPause = false;
		iconP1.changeIcon('poison');
		boyfriend.color = FlxColor.fromRGB(255, 160, 160);
		gotSlashed = true;
	...
	public function yell(key:Float)
	{
		isCameraOnForcedPos = true;
		FlxG.camera.zoom = 1.4;
		camGame.shake(0.02, key);
	}
